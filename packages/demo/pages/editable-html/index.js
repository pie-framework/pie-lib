import { Eh, getSerializer, defaultPlugins } from '@pie-lib/editable-html';
...
const serializer = getSerializer(_.compact(plugins.map(p => p.serialization)));
...
class Demo extends React.Component {
  ...
  constructor(props) {
    super(props);
    const value = serializer.deserialize(inputOptions[1].html);
    this.state = {
      value
      // showHighlight: false,
    ...
    return mounted ? (
      <div>
        ...
        <ul>
          <li>focus</li>
          <li>math updates</li>
          <li>no done button</li>
          <li>backward compatible api as much as possible</li>
        </ul>
        <h1>Step One: core editor using value w/ toolbar that uses focus/blur</h1>
        {/** 
           TO ADD: 
           pluginProps={{
             image: {
           ...
           height
            */}
        <Eh value={value} onChange={this.onChange} plugins={plugins} />
      </div>
    ) : (
      <div>loading...</div>
    );
