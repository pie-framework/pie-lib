  ...
  describe('getInterval', () => {
    ...
    describe('with bad params', () => {
      it('throws an error if min > max', () => {
        expect(() => {
          let result = utils.convertFrequencyToInterval({ min: 11, max: 10, tickFrequency: 1, betweenTickCount: 0 }, { interval: 10, major: 10 });
      ...
      it('throws an error if min = max', () => {
        expect(() => {
          let result = utils.convertFrequencyToInterval({ min: 10, max: 10, tickFrequency: 1, betweenTickCount: 0 }, { interval: 10, major: 10 });
          console.log('result: ', result);
        }).toThrow(Error);
      });
    });
    ...
    describe('with domain 0 -> 10', () => {
      ...
      it('throws an error if the tick frequency is less than 2', () => {
        expect(() => {
          let result = utils.convertFrequencyToInterval({ min: 0, max: 10, tickFrequency: 1, betweenTickCount: 0 }, { interval: 10, major: 10 });
          console.log('result: ', result);
        }).toThrow(Error);
      });
