...
const renderMath = (el, renderOpts) => {
  ...
  const renderMathAccessible = () => {
    ...
    if (
      ((!window.MathJax || window.MathJax.version !== MathJaxVersion) && !window.mathJaxSciptLoaded) ||
      (mathJaxCustomKey && mathJaxCustomKey !== mathRenderingAccessibleKEY)
    ) {
      renderOpts = renderOpts || defaultOpts();

      initializeMathJax(renderOpts, onLoadMathJaxScript);
    }

    if (isString && window.MathJax && window.mathJaxSciptLoaded) {
      try {
        MathJax.texReset();
